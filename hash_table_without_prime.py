...
def calculate_hash(key, base=31, mod=10**9): #baseは7以降の素数を使うとこのケースだと処理速度にほぼ変化がなかった。
    assert type(key) == str
...
class HashTable:
    ...
    def rehash_double(self):
        ...
        for item in old_buckets:
            while item:
                self.put_without_check(item.key, item.value)
                item = item.next
    ...
    def put(self, key, value):
        ...
        if self.item_count >= self.bucket_size * 0.8:
            self.rehash_double()
    ...
    def put_without_check(self, key, value): #rehashするときにputを使うとcheckに引っかかってエラーになってしまう。
        assert type(key) == str
        bucket_index = calculate_hash(key) % self.bucket_size
        item = self.buckets[bucket_index]
        while item:
